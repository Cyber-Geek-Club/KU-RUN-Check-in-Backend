...
from src.utils.token import create_access_token, verify_access_token
...
async def register_user(user: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    # Create user (user_crud.create_user should hash password internally)
...
async def login(credentials: UserLogin, db: AsyncSession = Depends(get_db)):
    ...
    # user_crud.verify_password should compare using argon2 (passlib)
...
async def get_users(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
    """
    Get all users (admin only)

    ดึงข้อมูลผู้ใช้ทั้งหมด
    """
    # Optionally enforce admin role check in user_crud or here
